// エラー文言
export const NOT_TEXT_ERROR = "テキストファイルではありませんでした";
export const NOT_EXCEL_ERROR = "Excelファイルではありませんでした";
export const SEND_FILE_ERROR = "file が送られていません";
export const TEMPLATE_ERROR = "テンプレートが選択されていません";

// 取得
export const FILENAME_ERROR = "ファイル名が取得できませんでした";
export const CODETEXT_ERROR = "コードが取得できませんでした";
export const EXCELFILE_ERROR = "Excelファイルが取得できませんでした";
export const JSONFILE_ERROR = "JSON ファイルが取得できませんでした";

// 読み込み
export const FILE_READ_ERROR = "ファイルが読み込みできませんでした";
export const MARKDOWN_ERROR = "マークダウンの読み込みに失敗しました";

// NOT FOUND
export const FILE_NOT_FOUND = "ファイルが見つかりません";
export const FILENAME_NOTFOUND = "ファイル名が本文から見つかりません";
export const CODEBLOCK_NOT_FOUND = "コードブロックが見つかりません";

// 汎用
export const UNKNOWN_ERROR = "不明なエラーが発生しました";
export const GENERATE_ERROR = "生成に失敗しました";
export const CHECK_ERROR = "チェック中にエラーが発生しました。";
